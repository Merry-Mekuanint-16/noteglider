...
export function buildHumanizationSystemMessage(isFreeUser: boolean = false): string {
  if (isFreeUser) {
    return `Mona Lisa’s Elements and Principles of Art 
Introduction
      ...
      async start(controller) {
        ...
        const sendPing = () => {
          ...
          if (now - lastPingTime >= PING_INTERVAL) {
            // Send a "thought" type ping which is already handled by UnifiedHomePage.tsx
            // This keeps the connection alive without affecting the UI text.
            ...
            lastPingTime = now;
            console.log("[Gemini Stream] Sent keep-alive ping to prevent Vercel timeout");
          }
        };

        try {
          while (true) {
            // Periodically send ping while waiting for reader.read()
            const readPromise = reader.read();
            const timeoutPromise = new Promise<{ done: boolean, value?: Uint8Array }>(resolve => {
              setTimeout(() => resolve({ done: false }), PING_INTERVAL);
            });

            const result = await Promise.race([readPromise, timeoutPromise]);
            
            if (!result.value && !result.done) {
              // This was a timeout race win, send ping and continue
              sendPing();
              continue;
            }
            ...
            lastPingTime = Date.now(); // Reset ping timer on actual data
