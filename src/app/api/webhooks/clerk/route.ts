...
export async function POST(req: Request) {
  ...
  if (eventType === 'user.created') {
    ...
    try {
      ...
      if (existingUser) {
        // User with this email already exists - link to existing account
        ...
        console.log(`User updated (linked to existing email): ${id}`)
      } else {
        // Create new user with 300 initial credits
        ...
        console.log(`User created with 300 credits: ${id}`)
      }
    } catch (error) {
